...
+ When unseated [hamzao]{.trn} comes between two letters that are joined, then it is written above the line that joins them, for example: [خَطِيءَة]{.ar} [xaTIEao]{.trn}. In this word, the [yAE]{.trn} [ي]{.ar} joins to the [tAE marbUTao]{.trn} [ة]{.ar}. So the unseated [hamzao]{.trn} is written above the joining line like so [⁠ء⁠]{.ar} [ـءـ]{.ar}.
...
Due to what appears to have been a limitation of typographical technology in the days of typewriters, metal typrography, and early digital typography, unseated [hamzao]{.trn} between two joining letters [ـءـ]{.ar} was usually written as seated on [yAE]{.trn} instead: [ـئـ]{.ar}. Because of this limitation we are now accustommed to seeing:
...
Fortunately, advancements in digital typography technology now allow us to revert back to the original rules. However, unfortunately, only very few computer fonts today actually implement this feature.
